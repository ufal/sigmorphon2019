...
class Network:
    ...
    def predict(self, dataset, args, probs_basename=None):
        import io
        ...
        while not dataset.epoch_finished():
            ...
            if probs_basename:
                targets.append(self.prediction_probs)

            predictions, *other_values = self.session.run(targets, feeds)

            if probs_basename:
                predicted_probs, *other_values = other_values
                for i in range(len(sentence_lens)):
                    for factor in args.factors:
                        probs[factor].append(predicted_probs[factor][i][:sentence_lens[i]])
    ...
    def evaluate(self, dataset_name, dataset, dataset_conllu, args):
        ...
        results = evaluate_2019_task2.manipulate_data(evaluate_2019_task2.input_pairs(dataset_conllu, conllu.split("\n")))
...
if __name__ == "__main__":
    ...
    if args.predict:
        ...
        do_not_log = {"exp", "max_sentence_len", "min_epoch_batches", "predict", "predict_probs", "rnn_cell", "threads"}
    ...
    if args.predict:
        ...
        dev = UMDataset("{}-dev.conllu".format(args.basename), train=train, shuffle_batches=False,
                        elmo=re.sub("(?=,|$)", "-dev.npz", args.elmo) if args.elmo else None)
        with open("{}-dev.conllu".format(args.basename), "r", encoding="utf-8") as dev_conllu_file:
            dev_conllu = dev_conllu_file.readlines()
        test = UMDataset("{}-covered-test.conllu".format(args.basename), train=train, shuffle_batches=False,
    ...
    if args.predict:
        ...
        conllu = network.predict(predict, args, probs_basename=args.predict_probs)
        print(conllu, end="")
    else:
        ...
        for i, (epochs, learning_rate) in enumerate(args.epochs):
            for epoch in range(epochs):
                ...
                metrics = network.evaluate("dev", dev, dev_conllu, args)
        ...
        for dataset_name, dataset in [("dev", dev), ("test", test)]:
            predicted_basename = "{}/{}-{}".format(args.logdir, os.path.basename(args.basename), dataset_name)
